...
const getActivity2025 = gql`
  query {
    user(login: "diegoSt23") {
      name
      contributionsCollection(
        to: "2025-12-31T01:01:00"
        from: "2025-01-01T01:01:00"
      ) {
        contributionCalendar {
          colors
          totalContributions
...
export const GitHub = () => {
  ...
  const { loading: loading2025, data: data2025 } = useQuery(getActivity2025);
  ...
  useEffect(() => {
    if (data2023?.user && data2022?.user && data2024?.user && data2025?.user) {
      ...
      const formattedData2025 =
        data2025.user.contributionsCollection?.contributionCalendar?.weeks
          .map((item: WeeksProps) => item.contributionDays)
          .flat()
          .map((item: WeekProps) => ({
            day: item.date,
      ...
      const totalContributions =
        ...
        data2024.user.contributionsCollection?.contributionCalendar
          .totalContributions +
        data2025.user.contributionsCollection?.contributionCalendar
          .totalContributions;

      setLocalData(uniqueArray);
      setTotalContributions(totalContributions);
    }
  }, [data2023?.user, data2022?.user, data2024?.user, data2025?.user]);

  return (
    <>
      <div className='mx-auto flex flex-col h-[50%] max-w-4xl items-center px-[1rem] pt-[5rem] pb-6 p-2 relative'>
        <h3 className='scroll-m-20 pb-2 text-4xl font-semibold tracking-tight first:mt-0 text-center'>
          {t('github.title')}
        </h3>
        ...
        <Card className='w-full py-4 lg:py-10 mt-8 mb-12'>
          {(!loading2023 || !loading2022 || !loading2024 || !loading2025) &&
          localData?.length ? (
            <>
              <div
                style={{
      ...
      <div className='relative flex h-[700px] w-full flex-col items-center justify-center overflow-hidden'>
        ...
        <div className='relative z-11 mt-4'>
          <ShinyButton onClick={handleRedirect} text={t('github.visit')} />
        </div>
      </div>
    </>
  );
